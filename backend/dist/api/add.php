...
if (array_filter($inDB)) {
  respond('Provided SKU is already used in DB', 400);
}
...
if ($product->type == "DVD") {
  if (empty($product->size)) {
    respond("Size is required for type DVD", 400);
  }
...
if ($product->type == "Furniture") {
  if (empty($product->width) || empty($product->height) || empty($product->length)) {
    respond("WxHxL dimensions are required for type Furniture", 400);
  }
...
if ($product->type == "Book") {
  if (empty($product->weight)) {
    respond("Weight is required for type Book", 400);
  }
